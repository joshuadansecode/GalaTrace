          ...
          <table className="w-full table-fixed text-sm border-collapse">
            <thead className="sticky top-0 bg-card z-10 border-b-2 border-border">
              <tr>
                ...
                <th className="text-left text-muted-foreground font-medium px-3 py-2.5 w-[7%] cursor-pointer hidden sm:table-cell border-r border-border" onClick={() => toggleSort('ticket_number')}>N° <SortIcon k="ticket_number" /></th>
                <th className="text-left text-muted-foreground font-medium px-3 py-2.5 w-[14%] cursor-pointer border-r border-border" onClick={() => toggleSort('ticket_type_id')}>Ticket <SortIcon k="ticket_type_id" /></th>
                <th className="text-left text-muted-foreground font-medium px-3 py-2.5 w-[7%] cursor-pointer hidden md:table-cell border-r border-border" onClick={() => toggleSort('filiere')}>Filière <SortIcon k="filiere" /></th>
                <th className="text-left text-muted-foreground font-medium px-3 py-2.5 w-[12%] hidden lg:table-cell border-r border-border">Vendeur</th>
                <th className="text-right text-muted-foreground font-medium px-3 py-2.5 w-[10%] hidden sm:table-cell border-r border-border">Payé</th>
                <th className="text-right text-muted-foreground font-medium px-3 py-2.5 w-[10%] cursor-pointer border-r border-border" onClick={() => toggleSort('remaining_balance')}>Reste <SortIcon k="remaining_balance" /></th>
                <th className="text-left text-muted-foreground font-medium px-3 py-2.5 w-[9%] hidden md:table-cell border-r border-border">Table</th>
            ...
            <tbody>
              {paginatedGuests.map((guest) => {
                ...
                return (
                  ...
                  >
                    <td className="px-3 py-2.5 font-medium overflow-hidden border-r border-border">
                    ...
                    <td className="px-3 py-2.5 text-muted-foreground text-xs hidden sm:table-cell border-r border-border">{guest.ticket_number || '—'}</td>
                    <td className="px-3 py-2.5 text-xs font-medium overflow-hidden truncate border-r border-border">{formatTicketType(guest.ticket_type_id)}</td>
                    <td className="px-3 py-2.5 text-xs hidden md:table-cell border-r border-border">
                      {guest.filiere || guest.annee
                        ? <span className="font-medium text-foreground">{(guest.filiere || '') + (guest.annee || '')}</span>
                        : <span className="text-muted-foreground">—</span>}
                    </td>
                    <td className="px-3 py-2.5 text-muted-foreground text-xs hidden lg:table-cell overflow-hidden truncate border-r border-border">{guest.seller?.full_name || guest.seller?.email || '—'}</td>
                    <td className="px-3 py-2.5 text-green-500 font-bold text-xs hidden sm:table-cell text-right whitespace-nowrap tabular-nums border-r border-border">{guest.total_paid?.toLocaleString()} F</td>
                    <td className={`px-3 py-2.5 font-bold text-xs text-right whitespace-nowrap tabular-nums border-r border-border ${guest.remaining_balance > 0 ? 'text-amber-500' : 'text-muted-foreground'}`}>{guest.remaining_balance?.toLocaleString()} F</td>
                    <td className="px-3 py-2.5 text-amber-500 text-xs hidden md:table-cell truncate border-r border-border">{table ? `${table.name}${seat ? ` #${seat.seat_number}` : ''}` : '---'}</td>
